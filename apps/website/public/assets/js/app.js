...
// Viewport Dark
{
  ...
  const items = $("[data-viewport-dark]")
    .toArray()
    .map((item) => ({
...
$("[data-toggle-loading]").on("click", function (e) {
  ...
  if (
    ...
    $this.hasClass("no-more-loading") ||
    !self.toggleLoadingClick ||
    !self.toggleLoadingInsertData
  )
    return;
...
document.querySelectorAll("[data-load-more]").forEach((anchorElement) => {
  ...
  anchor.toggleLoadingClick = () => {
    console.log("url", url);
    return fetch(url.replace(/%page%/g, ++page + "")).then((res) => res.text());
  };
