...
function WidgetCard(
  ...
  return (
    ...
    >
      <div
        className={cn((search || action) && "flex items-start justify-between", headerClassName)}
      >
        <div>
        ...
        {(search || action) && (
          <div className={cn("flex items-start justify-between ps-2 gap-4", actionClassName)}>
            {[action, search]}
          </div>
        )}
      </div>
